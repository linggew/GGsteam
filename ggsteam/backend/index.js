...
app.get("/api/most-popular", async (req, res) => {
  const query =
    "SELECT query_id, QueryName, HeaderImage FROM Game ORDER BY RecommendationCount DESC LIMIT 1000"
  pool.query(query, (error, results) => {
    if (error) {
      console.error("Database query error:", error)
      res.status(500).send("Database error")
...
app.post("/api/wishlist/removegame", async (req, res) => {
...
app.get("/api/reconmend/:id", async (req, res) => {
  const query =
    "CALL RecommendGames(?, @result); SELECT @result;"
  pool.query(query, [req.params.id], (error, results) => {
    if (error) {
      console.error("Database query error:", error)
      res.status(500).send("Database error")
