...
// Password reset endpoint using PUT
app.put("/api/reset-password", async (req, res) => {
  console.log(req.body);
  const { username, oldPassword, newPassword } = req.body;

  // Check if the username exists
  const userQuery = "SELECT user_id, password FROM User WHERE user_name = ?";
  pool.query(userQuery, [username], async (userError, userResults) => {
    if (userError) {
      console.error("User database query error:", userError);
      res.status(500).send("Database error");
      return;
    }
    ...
    if (userResults.length === 1) {
      ...
      if (isMatch) {
        ...
        // Update the password
        const updateQuery = "UPDATE User SET password = ? WHERE user_name = ?";
        pool.query(
          updateQuery,
        ...
        res.status(401).send("Old password verification failed");
      }
    } else {
...
app.put("/api/comment/updatecomment", async (req, res) => {
  ...
  try {
    ...
    pool.query(query, [text, review_id], (err, result) => {
      if (err) {
        console.error("Database query error:", err);
...
app.post("/api/pc/addpc", async (req, res) => {
  ...
  try {
    ...
    pool.query(query, [cpu, gpu, ram, storage, score], (err, result) => {
      ...
      const query2 = "INSERT INTO UserPC(user_id, pc_id) VALUES(?, ?)";
